...
export async function POST(req: NextRequest) {
  try {
    ...
    const [existing] = await db
      .select({ banner_url: configsTable.banner_url })
      .from(configsTable)
      .where(eq(configsTable.userId, session.userId));
    if (existing?.banner_url) {
      const oldKey = new URL(existing.banner_url).pathname.slice(1);
      await s3.send(
    ...
    const key = `banners/${session.userId}/${generateRandomFileName(file.name)}`;
    ...
    await db
      .update(configsTable)
      .set({ banner_url: url, updated_at: new Date() })
      .where(eq(configsTable.userId, session.userId));
...
export async function DELETE() {
  try {
    ...
    await db
      .update(configsTable)
      .set({ banner_url: null, updated_at: new Date() })
      .where(eq(configsTable.userId, session.userId));
